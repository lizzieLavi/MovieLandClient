...
function AddUser(props) {
  ...
  const [checkedPermissions, setCheckedPermissions] = useState([{name:"View Subscription" ,checked:false},{name: "Create Subscription",checked:false},
  {name:"Delete Subscription",checked:false},{name: "View Movie", checked: false} ,
  {name:"Create Movie" , checked:false},{name: "Delete Movie",checked: false}]);
  ...
  {  
    ...
    await axios.post('https://cinemaws.herokuapp.com/api/Permissions',{id:returnedLoginData.data.userId,permissions:permissionsArr},config)
    ...
    <div className='editUser'>
      <div className="border">
        <div className='userData'>
          ...
          <input className='data' id='firstName' type="text"  onChange={(e)=> setFirstName(e.target.value)} /><br/>
          <label  className='labels' for="lastName">Last Name: </label>
          <input className='data' id='lastName' type="text"  onChange={(e)=> setLastName(e.target.value)} /><br/>
          <label  className='labels' for="userName">User Name: </label>
          <input className='data' id='userName' type="text"  onChange={(e)=> setUserName(e.target.value)} /><br/>
          <label  className='labels' for="Password">Password: </label>
          <input className='data' id='Password' type="text"  onChange={(e)=> setPassword(e.target.value)} /><br/>
          <label className='labels' for="sessionTimeOut">Session Time Out </label>
          <input className='data' id='genres' type="text" onChange={(e)=>setSessionTimeOut(e.target.value)} /><br/>
        </div>
           ...
           {checkedPermissions.map((permission,index) => {
            return (
             <div className='checkboxItem'>
               <input  id={index} className='perCheckbox' onClick={checkPermission} type="checkbox"  value={permission.name} /> 
               <label className='checkLabel' for={index}>{permission.name}</label>
             </div> )
           })}
       </div>
